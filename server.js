...
app.get('/analyse', async (req, res) => {
  ...
  sf.stdout.on('data', (data) => {
    ...
    if (msg.includes('info depth') && msg.includes('score')) {
      ...
      if (score) {
        if (score[1] === 'cp') {
          const centipawn = !moves || moves.split(' ').length % 2 === 0 ? parseInt(score[2], 10) : -parseInt(score[2], 10);
          analysis.evaluation = (centipawn / 100).toFixed(2);
        } else if (score[1] === 'mate') {
          const mate = !moves || moves.split(' ').length % 2 === 0 ? parseInt(score[2], 10) : -parseInt(score[2], 10);
